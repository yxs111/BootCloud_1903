...
public class SwaggerConfig {
    private ApiInfo createAI(){
        return new ApiInfoBuilder().title("微服务初体验接口平台").description("基于SpringCloud实现的开心笑一笑").version("0.1")
    ...
    public Docket createD(){
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(createAI()).select().apis
                (RequestHandlerSelectors.basePackage("com.qfedu.cloud.consumer.controller")).build();
    }
}
